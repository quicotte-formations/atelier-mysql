...
FROM commande
         ...
         JOIN produit ON commande_produit.produit_id = produit.id
         JOIN marque ON produit.marque_id = marque.id
WHERE client.prenom = 'Angie' AND client.nom = 'Jolina' AND commande.etat = 'PAYEE'
...
FROM marque
         ...
         JOIN client ON commande.client_id = client.id
         JOIN categorie ON produit.categorie_id = categorie.id
WHERE client.nom = 'Smite' AND client.prenom = 'Willie' AND categorie.nom = 'Chapeaux'
ORDER BY Marque ASC;
...
(SELECT produit_id FROM commande
                            JOIN commande_produit ON commande.id = commande_produit.commande_id
                            JOIN client ON commande.client_id = client.id
 WHERE client.nom = 'Johannsen' AND client.prenom = 'Scarla')
INTERSECT
...
(SELECT marque_id FROM commande
                           JOIN commande_produit ON commande.id = commande_produit.commande_id
                           JOIN produit ON commande_produit.produit_id = produit.id
                           JOIN client ON commande.client_id = client.id
 WHERE client.nom = 'Clouni' AND client.prenom = 'Greg')
...
(SELECT marque_id FROM commande
                           ...
                           JOIN produit ON commande_produit.produit_id = produit.id
                           JOIN client ON commande.client_id = client.id
 WHERE client.nom = 'Petit' AND client.prenom = 'Brad');

-- 22 Les clients ayant commandé dans la catégorie Bijoux mais jamais
-- dans la marque Gucci.
-- Ils doivent avoir également commandé des Parfums mais
-- toujours dans la marque Versaz
-- Clients ayant commandé Bijoux
(SELECT client.id FROM commande
                           JOIN commande_produit ON commande.id = commande_produit.commande_id
                           JOIN produit ON commande_produit.produit_id = produit.id
                           JOIN categorie ON produit.categorie_id = categorie.id
                           JOIN client ON commande.client_id = client.id
...
(SELECT client.id FROM commande
                           ...
                           JOIN marque ON produit.marque_id = marque.id
                           JOIN client ON commande.client_id = client.id
 WHERE categorie.nom = 'Parfums' AND marque.nom = 'Versaz');

-- 23 Le nombre total de catégories, produits, marques, clients
SELECT
    (SELECT COUNT(*) FROM categorie) AS TotalCategories,
